...
class TelaresService {
  ...
  async saveDetalle(data, usuario) {
    ...
    return await this.telaresRepository.withTransaction(async () => {
      const procesoId = await this.telaresRepository.getProcesoTelaresId();
      ...
      for (const a of calidad.ancho) {
          ...
          if (!nominal && produccion.length > 0) {
              const orden = await this.telaresRepository.getOrdenEspecificaciones(produccion[0].orden_id);
      ...
      for (const inc of incidentes) {
          ...
          await this.telaresRepository.saveIncidente({
