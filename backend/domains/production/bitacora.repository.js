...
class BitacoraRepository {
  ...
  async getResumenProcesos() {
    // Ya no se consulta la base de datos para obtener tipos de proceso.
    // Se delega al ProcessRegistry en el Service.
    return [];
  }

  async getRegistrosByProceso(bitacoraId, procesoId) {
    return await this.db.query(`
      SELECT rt.*, op.codigo_orden, op.id as orden_id
      ...
      WHERE rt.bitacora_id = ? AND (le.proceso_id = ? OR rt.linea_ejecucion_id IS NULL)
