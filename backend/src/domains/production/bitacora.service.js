...
const saveProcesoData = async (data) => {
    ...
    for (const p of (produccion || [])) {
        let linea = await lineaEjecucionRepository.findByOrdenAndProceso(p.orden_id, proceso_id);
        if (!linea) {
            const resId = await lineaEjecucionRepository.create(p.orden_id, proceso_id);
            linea = { id: resId };
        }
        ...
        await registroTrabajoRepository.create({
    ...
    if (muestras) {
        for (const m of muestras) {
            await muestraRepository.create({
